...
MEDIA_TIMEOUT_IMAGE  = 40   # 图片/头像：40s 内下不完就放弃
MEDIA_TIMEOUT_VIDEO  = 60   # 视频文件可能大，保留 60s
WAYBACK_HTML_TIMEOUT = 60   # wayback HTML：保持 60s（限速时确实需要等）

# 默认并发与延迟（每个子命令可用 CLI 覆盖）
DEFAULT_WORKERS_HTML   = 7
DEFAULT_WORKERS_MEDIA  = 8   # pbs.twimg.com 比 wayback 宽松，可以更高并发
